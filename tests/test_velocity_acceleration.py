...
import src.velocity_acceleration as va
...
class TestCalcEscapeVelocity(object):
    def test_on_one_zero_value(self):
        with pytest.raises(ValueError) as e:
            va.calc_escape_velocity(0, 1)
        assert e.match(
            "Mass and radius values must be non-zero."), "Expected ValueError."
        with pytest.raises(ValueError) as e:
            va.calc_escape_velocity(1, 0)
        assert e.match(
            "Mass and radius values must be non-zero."), "Expected ValueError."

    def test_on_two_zero_values(self):
        with pytest.raises(ValueError) as e:
            va.calc_escape_velocity(0, 0)
        assert e.match(
            "Mass and radius values must be non-zero."), "Expected ValueError."

    def test_on_one_negative_value(self):
        with pytest.raises(ValueError) as e:
            va.calc_escape_velocity(-1, 1, earth_units=False)
        assert e.match(
            "Mass and radius values must be positive."), "Expected ValueError."
        with pytest.raises(ValueError) as e:
            va.calc_escape_velocity(1, -1, earth_units=False)
    ...
    def test_on_valid_values(self):
        # Earth values
        earth_theory = 11.2e3
        earth_calc = va.calc_escape_velocity(
            5.97e24, 6.371e6, earth_units=False)
        assert earth_calc == pytest.approx(
            earth_theory, 0.2), "Expected {0}, but got {1}.".format(earth_theory, earth_calc)
        # Jupiter values
        jupiter_theory = 59.5e3
        jupiter_calc = va.calc_escape_velocity(
            1.9e27, 1.42e8 / 2, earth_units=False)
        assert jupiter_calc == pytest.approx(
            jupiter_theory, 0.2), "Expected {0}, but got {1}.".format(jupiter_theory, jupiter_calc)
        # Sun values
        sun_theory = 615e3
        sun_calc = va.calc_escape_velocity(
            1.99e30, 6.957e8, earth_units=False)
        assert sun_calc == pytest.approx(
    ...
    def test_on_valid_values_earth_units(self):
        # Earth values
        earth_theory = 11.2e3
        earth_calc = va.calc_escape_velocity(1, 1, earth_units=True)
        assert earth_calc == pytest.approx(
            earth_theory, 0.2), "Expected {0}, but got {1}.".format(earth_theory, earth_calc)
        # Jupiter values
        jupiter_theory = 59.5e3
...
class TestCalcOrbitalVelocity(object):
    def test_on_valid_values(self):
        theory = 365.357
        calc = va.calc_orbital_velocity(2e30, 1e15, earth_units=False)
        assert calc == pytest.approx(
            theory), "Expected {0}, but got {1}.".format(theory, calc)
        theory = 1118.748
        calc = va.calc_orbital_velocity(20, 1000, earth_units=True)
        assert calc == pytest.approx(
            theory, 1), "Expected {0}, but got {1}.".format(theory, calc)
